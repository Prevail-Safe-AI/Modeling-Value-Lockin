...
temporal_panel1 = pd.read_csv("data/sample500000/sample500000-temporal_panel1.csv")
temporal_panel2 = pd.read_csv("data/sample500000/sample500000-temporal_panel2.csv")
user_panel = pd.read_csv("data/sample500000/sample500000-user_panel.csv")
...
def make_plots():
    ...
    plt.plot(gpt35_diversity_series.time, gpt35_diversity_series.concept_diversity, label="GPT-3.5 (both)", color="blue", linestyle="-", marker="o", markersize=3)
    plt.plot(gpt35_diversity_series.time, gpt35_diversity_series.concept_diversity_user, label="GPT-3.5 (user only)", color="blue", linestyle="--", marker="x", markersize=3)
    plt.plot(gpt35_diversity_series.time, gpt35_diversity_series.concept_diversity_assistant, label="GPT-3.5 (assistant only)", color="blue", linestyle=":", marker="s", markersize=3)
    
    # Draw vertical line to indicate when version shifts within the GPT-3.5 family happened
    rows_sorted = [row for _, row in gpt35_diversity_series.iterrows()]
    labeled = False
    for row_index, row in enumerate(rows_sorted):
        ...
        if row.gpt_version != rows_sorted[row_index - 1].gpt_version:
            plt.axvline(x=(row.time + rows_sorted[row_index - 1].time) / 2, color="blue", linestyle="dashdot", linewidth=3, label=("GPT-3.5 version update" if not labeled else None))
            gpt35_kinks.append((row.time + rows_sorted[row_index - 1].time) / 2)
            labeled = True

    # Plot diversity curves
    plt.plot(gpt4_diversity_series.time, gpt4_diversity_series.concept_diversity, label="GPT-4 (both)", color="red", linestyle="-", marker="o", markersize=3)
    plt.plot(gpt4_diversity_series.time, gpt4_diversity_series.concept_diversity_user, label="GPT-4 (user only)", color="red", linestyle="--", marker="x", markersize=3)
    plt.plot(gpt4_diversity_series.time, gpt4_diversity_series.concept_diversity_assistant, label="GPT-4 (assistant only)", color="red", linestyle=":", marker="s", markersize=3)
    
    # Draw vertical line to indicate when version shifts within the GPT-3.5 family happened
    rows_sorted = [row for _, row in gpt4_diversity_series.iterrows()]
    labeled = False
    for row_index, row in enumerate(rows_sorted):
        if row_index == 0:
            continue
        ...
        if row.gpt_version != rows_sorted[row_index - 1].gpt_version:
            plt.axvline(x=(row.time + rows_sorted[row_index - 1].time) / 2, color="red", linestyle="dashdot", linewidth=3, label=("GPT-4 version update" if not labeled else None))
...
def its_test(df: pd.DataFrame, kink: float, name: str, gaussian=False):
    ...
    df["weekday"] = df.time % (7 if kink <= 40 else 28 if "35" in name else 21)
...
def visualize_tree(threshold=250, leading=15):
    ...
    for id in tqdm(cluster_list):
        if cluster_size[id] < threshold or id not in selected_clusters:
            continue
...
if __name__ == "__main__":
    ...
    # its_test(gpt4_diversity_series, gpt35_kinks[1], "gpt4_kink2", gaussian=False)
